...
void base_fill(int m,int n,vector<int>& poly,vector<int>& Rbase, multimap<int,int>& Abase,multimap<int, int>& Qbase){
	...
	//filling A_base
	for(int i=0;i<2*sz/3;i++){
		for(int j=0;j<prime[2*sz/3];j++){
			int p = prime[i];
			if(j==0){cout<<p<<" ";}
	...
	//filling Q_base
	for(int i=2*sz/3 + 1;i<sz;i++){
		for(int j=0; j<prime[sz-1];j++){
			int q = prime[i];
...
void A_smooth(multimap<int,int>& Abase,multimap<int,int>& siv,multimap<int,int>& siv2,vector<int>& poly){
	...
	for(;it!=siv.end();++it){
		...
		for(;itr!=Abase.end();itr++){
			...
			int p=(*itr).second;
			int lhs=a%p;
			int rhs=(-1*b*((*itr).first))%p;
			if(lhs<0){lhs=lhs+p;}
			if(rhs<0){rhs=rhs+p;}
			if(lhs==rhs){mult_p.push_back(p);}
